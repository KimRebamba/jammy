...
<div class="bg-slate-950/40 rounded-2xl border border-slate-800 p-4">
    ...
    <div class="flex items-center justify-between mb-2">
        <h2 class="text-xl font-semibold text-slate-50">Employees</h2>
        <a href="/admin/employees/create" class="inline-flex items-center px-3 py-1.5 rounded-lg bg-amber-500 text-slate-900 text-sm font-semibold hover:bg-amber-400 transition-colors">
            Add Employee
        </a>
    </div>

    <form method="GET" class="mb-3 flex items-center justify-end gap-2 text-xs text-slate-300">
        <label for="deleted-filter-employees">Show:</label>
        <select id="deleted-filter-employees" name="deleted" class="rounded-md border border-slate-700 bg-slate-900/80 text-xs px-2 py-1" onchange="this.form.submit()">
    ...
    <form method="post" action="/admin/employees/batch">
        @csrf

        <div class="rounded-xl border border-slate-800/80 bg-slate-950/60">
            {!! $dataTable->table(['class' => 'min-w-full text-sm text-left', 'id' => 'employees-table']) !!}
        </div>
