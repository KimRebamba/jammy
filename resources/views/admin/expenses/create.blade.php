...
<div class="max-w-3xl mx-auto">
    <div class="bg-slate-900/70 border border-slate-700/60 rounded-2xl shadow-xl p-6">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-xl font-semibold text-slate-50">Add Expense</h1>
        ...
        <form action="/admin/expenses" method="post" class="space-y-3">
            @csrf
            <p>Type:
                <select name="expense_type" class="mt-1 px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 w-full text-sm">
                    <option value="inventory_purchase" {{ old('expense_type') == 'inventory_purchase' ? 'selected' : '' }}>Inventory Purchase</option>
                    <option value="shipping" {{ old('expense_type') == 'shipping' ? 'selected' : '' }}>Shipping</option>
            ...
            <p>Description: <input type="text" name="description" value="{{ old('description') }}" class="mt-1 px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 w-full text-sm"></p>
            <p>Amount: <input type="text" name="amount" value="{{ old('amount') }}" class="mt-1 px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 w-full text-sm"></p>
            <p>Status:
            ...
            <p>Due Date: <input type="text" name="due_date" value="{{ old('due_date') }}" class="mt-1 px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 w-full text-sm"></p>
            <p>Paid Date: <input type="text" name="paid_date" value="{{ old('paid_date') }}" class="mt-1 px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 w-full text-sm"></p>
